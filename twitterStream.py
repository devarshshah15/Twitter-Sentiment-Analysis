...
def make_plot(counts):
    ...
    """
    positive = [item[1][1] for item in counts if not (not item)]
    negative = [item[0][1] for item in counts if not (not item)]
    #positive = [];
    #negative = [];
    #for i in counts:
    #    if not (not i):
    #        positive.append(i[1][1])
    #        negative.append(i[0][1]) 
...
def stream(ssc, pwords, nwords, duration):
    ...
    totalCount = word_counts.updateStateByKey(updateFunction)
    totalCount.pprint()
